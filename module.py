...
def generer_masque():
    ...
    # Debut
        # Création du masque
        # Demander 4 couleurs à deviner à l'utilisateur
            # Initialiser la lst_nmbre
        #créer boucle qui nous permettra de choisir 4 couleur à la fois (répéter 4 fois le processus)
        #créer une deuxième boucle + condition
            # faire la gestion d'erreurs ( ne pas entrer plusieurs couleurs à la fois, entrer une couleur qui est dans la liste)
        # Fin pour
        #return masque
    # Fin
    ...
    for i in range(4):
        while True:
            couleur = input(f"quelle est la {i+1}e couleur de la combinaison?: ").strip().lower()

            if couleur not in lst_nbre:
                print("Entre UNE seule couleur, pas plusieurs ou seulement les options qui sont présentes dans la liste.")
                continue
            else:
